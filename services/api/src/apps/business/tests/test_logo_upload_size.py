"""
Tests para la validación de tamaño en BusinessLogoUploadView.

Cubre:
- Archivo > 5 MB → 400 con mensaje "El archivo supera el límite de 5 MB."
- Archivo válido < 5 MB → 200 (respuesta con branding actualizado)
- Archivo exactamente en el límite (5 MB exactos) → 200 (borde: no debe rechazar)
"""
...
URL = '/api/v1/settings/branding/upload-logo/'
...
class BusinessLogoUploadSizeTests(APITestCase):
    """Valida que la vista rechace archivos mayores a 5 MB."""

    def setUp(self):
        self.user = User.objects.create_user(
            username='logo_upload_tester',
            email='logo_upload@test.com',
            password='testpass123',
        )

    def _bootstrap_business(self) -> Business:
        """Crea Business + Subscription activa + Membership owner y autentica."""
        business = Business.objects.create(name='Biz Logo Test')
        # 'starter' plan con service vacío → service defaults to 'gestion'
        # → owner tiene 'manage_commercial_settings'
    ...
    def test_oversized_file_returns_400(self):
        ...
        response = self.client.post(
            URL,
            {'file': oversized_file, 'type': 'horizontal'},
            format='multipart',
        )
    ...
    def test_valid_file_under_limit_returns_200(self):
        ...
        response = self.client.post(
            URL,
            {'file': valid_file, 'type': 'horizontal'},
            format='multipart',
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertIn('logo_horizontal_url', response.data)

    def test_file_at_exact_limit_is_allowed(self):
        """Un archivo de exactamente 5 MB no debe ser rechazado (límite exclusivo)."""
        self._bootstrap_business()

        # 5 MB exactos — el check es > 5MB, no >= 5MB
        ...
        response = self.client.post(
            URL,
            {'file': exact_file, 'type': 'square'},
            format='multipart',
        )

        # No debe rechazar por tamaño; puede fallar por contenido no siendo
        # una imagen válida, pero el status no debe ser 400 por size.
        self.assertNotEqual(
