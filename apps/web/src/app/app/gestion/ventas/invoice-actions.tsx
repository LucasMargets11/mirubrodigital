...
export function InvoiceActions({
    ...
    const defaultSeries = useMemo(() => availableSeries.find((serie) => serie.is_active)?.code ?? availableSeries[0]?.code ?? '', [availableSeries]);
        ...
        return (
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p className="mt-2 text-sm text-slate-500">
                    Tu plan actual no incluye los comprobantes digitales. Actualizá para emitir facturas desde las ventas.
                </p>
            </section>
        );
    }
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <h2 className="text-xl font-semibold text-slate-900">
                        {existingInvoice ? 'Factura emitida' : 'Generá tu factura en un clic'}
                    </h2>
                    <p className="text-sm text-slate-500">Factura vinculada a la venta #{saleNumber}.</p>
                </div>
            ...
            {existingInvoice ? (
                ...
                <p className="mt-4 text-sm text-slate-500">
                    Se generará una factura con los productos de la venta. Es un comprobante digital interno, no fiscal.
                </p>
            )}
            {!canIssue && !existingInvoice ? (
                <p className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-700">
                    No tenés permiso para generar facturas. Pedí acceso a un administrador.
                </p>
            ) : null}
            <Modal open={open} title="Generar factura" onClose={() => setOpen(false)}>
                <form className="space-y-4" onSubmit={handleSubmit}>
                    ...
                    <div className="flex items-center justify-end gap-3">
                        ...
                        <button
                            type="submit"
                            disabled={issueInvoiceMutation.isPending || !form.series_code}
                        ...
                        >
                            {issueInvoiceMutation.isPending ? 'Generando...' : 'Confirmar emisión'}
                        </button>
                    </div>
                </form>
            </Modal>
